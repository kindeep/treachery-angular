...
async function collectionToArray<E>(
...
async function _startGame(gameId: string, creatorUid: string) {
  ...
  await gameDoc
    .collection("users")
    .doc(creatorUid)
    .set({ murderer }, { merge: true });

  await gameDoc
    .collection("users")
    .doc(murderer.uid)
    .set({ isMurderer: true }, { merge: true });

  sendForensicMessage(
    gameId,
    "Game started! Murderer, select your cards. Don't let anyone else find out!"
  );
}

async function _selectMurdererCards(
  gameId: string,
  murdererUid: string,
  clueCardName: string,
  meansCardName: string
  ...
  sendForensicMessage(
    gameId,
    "Murderer selected their cards. Time to figure out who he is. Good luck!"
  );
}
...
exports.addPlayer = functions.https.onCall(
  async ({ gameId, playerName }, context) => {
    if (context && context.auth) {
      const uid = context.auth.uid;
      console.log(uid);
...
exports.selectMurdererCards = functions.https.onCall(
  async ({ gameId, clueCardName, meansCardName }, context) => {
    if (context && context.auth) {
      const uid = context.auth.uid;
      console.log(uid);
...
exports.makeGuess = functions.https.onCall(
  async ({ gameId, clueCardName, meansCardName, murdererUid }, context) => {
    const gameDoc = db.collection("games").doc(gameId);
    if (context.auth) {
      const uid = context.auth.uid;
      const newGuess = {
      ...
      const forensicPrivateDoc = gameDoc
        .collection("users")
        .doc(game.creatorUid);
      const forensicPrivateData = (
        await forensicPrivateDoc.get()
      ).data() as ForensicPrivateData;
      const correct =
      ...
      sendMessage(
        gameId,
        `I think '${guessedPlayerData.name}' is the murderer, with clue '${clueCardName}' and means '${meansCardName}'.`,
        MessageType.GUESS,
        uid
      );
...
async function checkAndEndGame(gameId: string) {
  const gameDoc = db.collection("games").doc(gameId);
  const game = (await gameDoc.get()).data() as Game;

  const guessesCol = gameDoc.collection("guesses");
  const guesses: Guess[] = await collectionToArray<Guess>(guessesCol);
  ...
  // Check if each player except the murderer has guessed

  const guessedPids = guesses.map((guess) => guess.guessedByUid);

  let end = true;

  for (let player of players) {
    if (
      player.uid !== forensicPrivateData.murderer.uid &&
      !guessedPids.includes(player.uid)
    ) {
      // There is a guess left
      end = false;
    }
  }
